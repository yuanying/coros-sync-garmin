# Copy from https://github.com/XiaoSiHwang/garmin-sync-coros/blob/master/scripts/coros/coros_client.py
import urllib3
import json
import hashlib
...
class CorosClient:
    ...
    def uploadActivity(self, oss_object, md5, fileName, size):
        ...
        upload_url = "https://teamapi.coros.com/activity/fit/import"
    ...
    def getActivities(self, size:int, page:int):
        self.checkToken()
        activitys_url = f"https://teamapi.coros.com/activity/query?size={size}&pageNumber={page}"
        headers = {
          "Accept":       "application/json, text/plain, */*",
          "accesstoken": self.accessToken,
        }
    ...
    def downloadActivitie(self, id, sport_type):
       self.checkToken()
       ## 文件下载链接
       get_activity_download_url = f"https://teamapi.coros.com/activity/detail/download?labelId={id}&sportType={sport_type}&fileType=4"
       headers = {
          "Accept":       "application/json, text/plain, */*",
          "accesstoken": self.accessToken,
       }
